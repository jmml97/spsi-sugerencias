...
\item En ocasiones será necesario poner en correspondencia los símbolos del alfabeto con números naturales. Si $\mathcal{A}$ es un alfabeto de $n \in \omega^\ast$ símbolos, consideraremos fijada una función biyectiva $f: \mathcal{A} \to n$. De esta forma, si $s \in exp(\mathcal{A})$ nos referiremos a $f(s)$ como $\langle f(s_j) \rangle_j$.
...
\begin{sol}
El criptosistema de ElGamal es un criptosistema de clave pública propuesto por Tahir ElGamal en 1985. Está basado en el protocolo de intercambio de claves de Diffie-Hellman.
...
Supongamos que Alice quiere compartir con Bob un mensaje \(0\leq m \leq q\). Para ello, recoge la clave pública de Bob ($y_B$), y calcula $K = y_B ^{x_A} \bmod p$, donde $x_A$ es su clave privada. Finalmente envía a Bob la pareja \(c = \langle y_{A}, mK_{AB} \rangle\), donde \(y_{A} = g^{x_{A}} \bmod{q}\).
\subsubsection*{Descifrando el mensaje}

Cuando \textbf{Bob} recibe el mensaje de \textbf{Alice} tiene una pareja de números $c = \langle c_1, c_2 \rangle$ menores que \(q\). El primero de ellos lo eleva a su número secreto \(c_{1}^{x_{B}} = (g^{x_{A}})^{x_{B}} = K_{AB} \bmod {q}$. Como estamos en el grupo de las unidades, todos los elementos son invertibles, así que con el algoritmo extendido de Euclides calcula un inverso de \(K_{AB}\) módulo \(q\) y multiplica la segunda componente del mensaje cifrado por dicho inverso \(c_{2}K_{AB}^{-1} = (mK_{AB})K_{AB}^{-1} = m\), lo que le permite recuperar el mensaje de \textbf{Alice}.

\subsubsection*{Vulnerabilidades}

Como vemos, en este esquema al enviar cada mensaje \textbf{Alice} debe elegir un número secreto \(x_{A}\) \textbf{distinto}. Esto se debe a que si un atacante supiera el contenido de un primer mensaje \(m_{1}\) podría calcular el contenido de las porciones siguientes de la siguiente forma.
...
\end{align*}
Entonces es fácil calcular \(m_2m_1^{-1}\equiv c_{22}c_{21}^{-1} \bmod{p}\), de donde se deriva inmediatamente \(m_{2}\) (multiplicando por \(m_{1}\)).  \\
